...
def load_ddl(iamconnectioninfo, working_dir):
    if iamconnectioninfo.tpcds != '' and iamconnectioninfo.tpcds != '':
        ...
        with connect(dbname='postgres', user='ec2-user') as conn:
            cursor = conn.cursor()
            cursor.execute(
                'CREATE TABLE IF NOT EXISTS load_status(tablename VARCHAR(128), dataset VARCHAR(15), '
...
def load_tpcds(num_worker_process):
    ...
    for i in range(num_worker_process):
        tpcds_worker_process = mp.Process(target=load_worker,
                                          args=(tpcds_table_queue, dataset),
                                          daemon=True,
...
def load_worker(queue, data_set):
    ...
    while True:
        ...
        with connect(dbname='postgres', user='ec2-user') as conn:
            cursor = conn.cursor()
            cursor.execute(
                'INSERT INTO load_status(tablename,dataset,status,load_start,querytext) '
        ...
        with connect(dbname='postgres', user='ec2-user') as conn:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE load_status SET status=\'complete\',load_end=timezone(\'utc\', now()), '
